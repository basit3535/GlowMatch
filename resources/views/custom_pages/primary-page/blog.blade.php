            ...
            <div class="blog-layout">
                    ...
                    <div class="blog-featured-wrap">
                        <div class="blog-feat-img">
                            @if ($data->featured->image)
                                <img src="{{ asset('storage/' . $data->featured->image->path) }}"
                        ...
                        <div class="blog-feat-body">
                            ...
                            <a href="{{ route('show.single_blog', ['category' => $data->featured->category->slug, 'slug' => $data->featured->slug]) }}"
                                class="btn-outline" style="font-size:13px;padding:10px 22px;">Read Article →</a>
                        </div>
                    </div>
                @endif
                ...
                <div class="blog-cards" id="blog-cards">
                    @forelse($data->blogs as $blog)
                        <a
                            href="{{ route('show.single_blog', ['category' => $blog->category->slug, 'slug' => $blog->slug]) }}">
                            <div class="blog-card">
    ...
    <script>
            ...
            function fetchBlogs(category, page = 1) {
                ...
                fetch(url)
                    ...
                    .then(data => {
                        ...
                        let featuredHtml = data.featured ?
                            generateFeaturedHtml(data.featured) :
                            '';
        ...
        `;
            ...
            function generateFeaturedHtml(featured) {
                const imageHtml = featured.image ?
                    `<img src="/storage/${featured.image.path}" alt="${featured.title}">` : '🍂';
                const category = featured.category || 'Colour Theory';
                const readingTime = featured.reading_time || '5 min read';
                const date = new Date(featured.created_at).toLocaleDateString('en-US', {
                    month: 'short',
                    day: '2-digit',
                    year: 'numeric'
                });
        ...
        `;
            ...
            filters.addEventListener('click', function(e) {
                console.log('Filter clicked:', e); // Debugging line
                ...
                console.log('Filter clicked:', category); // Debugging line
                // Update active state
