...
<div class="page" id="page-bodytypes">
  <div class="bodytype-hero">
    ...
    <p style="font-size:16px;color:var(--muted);max-width:560px;line-height:1.75;margin-top:10px;">{{ $data->content->body_hero_desc->value ?? '' }}</p>
    <div style="margin-top:24px;">
      <a class="btn-primary" href="{{ route('show.custom_pages', ['slug' => 'test']) }}">{{ $data->content->body_hero_btn->value ?? '' }}</a>
    </div>
  </div>
  ...
  <section class="section" style="background:var(--blush-light);text-align:center;">
    ...
    <div class="hero-btns" style="justify-content:center;margin-top:28px;">
      <a class="btn-primary" href="{{ route('show.custom_pages', ['slug' => 'test']) }}">{{ $data->content->crossover_btn1->value ?? '' }}</a>
      <a class="btn-outline" href="{{ route('show.custom_pages', ['slug' => 'seasons']) }}">{{ $data->content->crossover_btn2->value ?? '' }}</a>
    </div>
  </section>
  ...
  <section class="dressup-section">
    ...
    <p class="section-sub" style="text-align:center;margin-bottom:32px;">{{ $data->content->interactive_sub->value ?? '' }}</p>
    <div class="dressup-grid">
      <div class="dressup-list" id="dl">
        <div class="dl-item active" onclick="selectDressup(this,'hourglass')">
          <div class="dl-icon">⧖</div>
          <div><div class="dl-name">{{ $data->content->interactive_hourglass_name->value ?? '' }}</div><div class="dl-sub">{{ $data->content->interactive_hourglass_sub->value ?? '' }}</div></div>
        </div>
        <div class="dl-item" onclick="selectDressup(this,'pear')">
          <div class="dl-icon">🍐</div>
          <div><div class="dl-name">{{ $data->content->interactive_pear_name->value ?? '' }}</div><div class="dl-sub">{{ $data->content->interactive_pear_sub->value ?? '' }}</div></div>
        </div>
        <div class="dl-item" onclick="selectDressup(this,'apple')">
          <div class="dl-icon">🍎</div>
          <div><div class="dl-name">{{ $data->content->interactive_apple_name->value ?? '' }}</div><div class="dl-sub">{{ $data->content->interactive_apple_sub->value ?? '' }}</div></div>
        </div>
        <div class="dl-item" onclick="selectDressup(this,'rectangle')">
          <div class="dl-icon">▭</div>
          <div><div class="dl-name">{{ $data->content->interactive_rectangle_name->value ?? '' }}</div><div class="dl-sub">{{ $data->content->interactive_rectangle_sub->value ?? '' }}</div></div>
        </div>
        <div class="dl-item" onclick="selectDressup(this,'inverted')">
          <div class="dl-icon">▽</div>
          <div><div class="dl-name">{{ $data->content->interactive_inverted_name->value ?? '' }}</div><div class="dl-sub">{{ $data->content->interactive_inverted_sub->value ?? '' }}</div></div>
        </div>
      </div>
      <div class="dressup-preview">
        <div class="dp-top" style="background:linear-gradient(145deg,#FDF0EA,#F5D0BC);" id="dp-img">⧖</div>
        <div class="dp-body">
          <div class="dp-title" id="dp-title">{{ $data->content->interactive_preview_title->value ?? '' }}</div>
          <div class="dp-tips" id="dp-tips">
            <div class="dp-tip"><span>✦</span>{{ $data->content->interactive_preview_tip1->value ?? '' }}</div>
            <div class="dp-tip"><span>✦</span>{{ $data->content->interactive_preview_tip2->value ?? '' }}</div>
            <div class="dp-tip"><span>✦</span>{{ $data->content->interactive_preview_tip3->value ?? '' }}</div>
            <div class="dp-tip"><span>✦</span>{{ $data->content->interactive_preview_tip4->value ?? '' }}</div>
          </div>
        </div>
      </div>
    </div>
  ...
  <section class="section" style="max-width:720px;margin:0 auto;padding-bottom:80px;">
    ...
    <div style="display:flex;flex-direction:column;gap:24px;">
      <div>
        <h3 style="font-size:17px;font-weight:600;margin-bottom:8px;color:var(--charcoal);">{{ $data->content->faq_q1->value ?? '' }}</h3>
        <p style="font-size:15px;color:var(--muted);line-height:1.75;">{{ $data->content->faq_a1->value ?? '' }}</p>
      </div>
      <div>
        <h3 style="font-size:17px;font-weight:600;margin-bottom:8px;color:var(--charcoal);">{{ $data->content->faq_q2->value ?? '' }}</h3>
        <p style="font-size:15px;color:var(--muted);line-height:1.75;">{{ $data->content->faq_a2->value ?? '' }}</p>
      </div>
      <div>
        <h3 style="font-size:17px;font-weight:600;margin-bottom:8px;color:var(--charcoal);">{{ $data->content->faq_q3->value ?? '' }}</h3>
        <p style="font-size:15px;color:var(--muted);line-height:1.75;">{{ $data->content->faq_a3->value ?? '' }}</p>
      </div>
    </div>
  </section>
